package com.banking.management.authservice.controller;
...
import com.banking.management.authservice.model.AuthRequest;
import com.banking.management.authservice.model.AuthResponse;
import com.banking.management.authservice.model.LoginResponse;
import com.banking.management.authservice.model.ResultMessage;
...
//	}
	...
	public ResponseEntity<LoginResponse> login(@RequestBody AuthRequest loginRequest) throws Exception {
		...
		String roles = userService.getUser(loginRequest.getUserName()).isPresent() ? userService.getUser(loginRequest.getUserName()).get().getRole() : null;
		LoginResponse loginResponseDTO;
		if(token.isEmpty()) {
			loginResponseDTO = new LoginResponse(loginRequest.getUserName(),token,roles,new ResultMessage(401,"Invalid Credentials!"));
		}else {
			loginResponseDTO = new LoginResponse(loginRequest.getUserName(),token,roles,new ResultMessage(200,"Success"));
